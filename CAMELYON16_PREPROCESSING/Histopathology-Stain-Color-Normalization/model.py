...
class CNN(object):
  ...
  def __call__(self, D):
    with tf.variable_scope(self.name, reuse=self.reuse):
        ...
        #G_conv1 = tf.Print(G_conv1,[G_conv1],"GCONV", summarize =200)
...
class DCGMM(object):
  def __init__(self, sess, config, name, is_train):
    ...
    #self.D = tf.Print(self.D,[self.D],"self.D", summarize=200)

    self.E_Step = CNN("E_Step", config, is_train=self.is_train)
    self.Gama, D, conv  = self.E_Step.__call__(self.D)

    #self.Gama = tf.Print(self.Gama,[self.Gama],"self.Gama",summarize=200)
    ...
    if self.is_train:
      ...
      self.loss = tf.Print(self.loss,[self.loss],"Loss print")
    ...
    tf.summary.image("1.Input_image", self.X_rgb*255.0, max_outputs=10)
    tf.summary.image("2.Hsd_image",self.X_hsd, max_outputs=10)
    tf.summary.image("3.Gamma_image",  Msk, max_outputs=10)
    tf.summary.image("4.Density_image", self.D*255.0, max_outputs=10)
  ...
  def fit(self, X):
#    if str(type(self.sess)).find('debug') == -1:
