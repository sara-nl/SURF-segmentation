...
def update_learning_rate_schedule_parameters(params):
  ...
  batch_size = params['batch_size'] * hvd.size() #* params['num_shards']
...
class CosineLrSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
  ...
  def __call__(self, step):
    ...
    cosine_lr = 0.5 * self.adjusted_lr * (
        1 + tf.cos(math.pi * tf.cast(step, tf.float32) / self.decay_steps)) + 1e-7
...
class CyclicLrSchedule(tf.keras.optimizers.schedules.LearningRateSchedule):
    ...
    def __init__(self, base_lr=0.001, max_lr=0.006, step_size=2000., gamma=1.):
        """Build a CyclicLrSchedule.

        Args:
          base_lr: `float`, The learning rate to start at.
          max_lr: `float`, The maximum learning rate.
          step_size: `float`, The number of steps for increasing and decreasing the LR
          gamma: `float`, The exponential decay factor
        """
        super().__init__()
        logging.info('LR schedule method: cyclic')
    ...
    def __call__(self, step):
        ...
        learning_rate = self.base_lr + (self.max_lr - self.base_lr) * tf.math.maximum(0.0, (1 - x)) * self.scale_fn(step)
...
def learning_rate_schedule(params):
  ...
  if lr_decay_method == 'cyclic':
    return CyclicLrSchedule(params['lr_warmup_init'],
...
def get_optimizer(params):
  ...
  if params['optimizer'].lower() == 'sgd':
    ...
    optimizer = tf.keras.optimizers.SGD(1e-3, momentum=params['momentum'],clipnorm=params['clip_gradients_norm'])
  elif params['optimizer'].lower() == 'adam':
    logging.info('Use Adam optimizer')
    optimizer = tf.keras.optimizers.Adam(clipnorm=params['clip_gradients_norm'])
  else:
...
class DisplayCallback(tf.keras.callbacks.Callback):
  ...
  def on_epoch_end(self, epoch, logs=None):
    ...
    if epoch % self.update_freq == 0:
        ...
        self.draw_inference(epoch)
        
      # self.executor.submit(self.draw_inference, epoch)

  @tf.function
  def inference(self):
    # return self.model(self.valid_images, training=False)
    
    return self.train_model(self.valid_images,training=True)
  ...
  def draw_inference(self, epoch):
    ...
    if hvd.rank() == self.save_worker:
        ...
        with self.file_writer.as_default():
          ...
          tf.summary.scalar(f' Worker {hvd.rank()}:Learning Rate', self.train_model.optimizer.get_config()['learning_rate'],step=epoch)
        print(f" Worker {hvd.rank()}: Finished writing summaries")

      
      

def get_callbacks(params, train_sampler, valid_sampler, profile=False):
    ...
    if hvd.rank() == 0:
        ...
        #    monitor='val_loss', min_delta=1e-2, patience=10, verbose=1)
    ...
    display_callback = DisplayCallback(
              train_sampler,
              valid_sampler,
              os.path.join(params['log_dir'], 'test'))
