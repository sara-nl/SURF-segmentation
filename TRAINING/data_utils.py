...
class SurfSampler(PreProcess):
   ...
   mpirun -map-by ppr:4:node -np 4 -x LD_LIBRARY_PATH -x PATH python -u train.py \
    ...
    def __init__(self, opts):
        ...
        self.train_paths = shuffle(
            [(difflib.get_close_matches(label.split('/')[-1], slides, n=1, cutoff=0.1)[0], label) for label in labels])
        ...
        assert hvd.size() < len(
            self.valid_paths), "WARNING: {hvd.size()} workers will share {len(self.valid_paths)} images"
    ...
    def trainer(self, image, mask_image, img_reg, mask_reg, numpy_batch_patch, numpy_batch_mask, save_image):
        ...
        for i in range(int(self.batch_size * (1 - self.tumor_ratio))):
            ...
            if rank00(): print(
                f"Sample {self.patch_size} x {self.patch_size} from contours = {h}" + f" by {w} in {time.time() - t1} seconds")
            numpy_batch_patch.append(patch)

            mask = mask_reg.fetch(x_topleft, y_topleft, self.patch_size, self.patch_size)
            numpy_batch_mask.append(
                np.ndarray((self.patch_size, self.patch_size, mask_image.get('bands')), buffer=mask, dtype=np.uint8))

            try:
                # Draw the rectangles of sampled images on downsampled rgb
                save_image = cv2.drawContours(save_image, self.contours, -1, (0, 255, 0), 1)
                save_image = cv2.rectangle(save_image,
                                           (int(x_topleft // self.mag_factor), int(y_topleft // self.mag_factor)),
                                           (int((x_topleft + self.patch_size) // self.mag_factor),
                                            int((y_topleft + self.patch_size) // self.mag_factor)),
                                           (0, 255, 0), 2)
            except:
                pass
    ...
    def tester(self, image, mask_image, img_reg, mask_reg, numpy_batch_patch, numpy_batch_mask, save_image, past_coords,
               past_wsi):
        ...
        Image.fromarray(save_image[..., :3]).save(
            os.path.join(self.log_image_path, self.cur_wsi_path[0].split('/')[-1].replace(self.slide_format, 'png')))
        dataset = tf.data.Dataset.from_tensor_slices((np.array(numpy_batch_patch), np.array(numpy_batch_mask)))
        dataset = dataset.apply(
            tf.data.experimental.map_and_batch(
                map_func=lambda im, msk: SurfSampler._load(im, msk, augment=False),
    ...
    def get_next(self, train=True, past_coords=[], past_wsi=[]):
        ...
        if train:
            while not self.contours_train or not self.contours_tumor:
                ...
                self.rgb_image_pil = self.wsi.read_region((0, 0), self.level_used,
                                                          self.wsi.level_dimensions[self.level_used])
                self.mask_pil = self.mask.read_region((0, 0), self.level_used,
                                                      self.wsi.level_dimensions[self.level_used])
            ...
            while not self.contours_valid or not self.contours_tumor:
                ...
                if past_wsi:
                    ...
                    if rank00(): print(
                        f"Evaluated {len(past_wsi) * hvd.size()} / {len(past_wsi) * hvd.size() + len(self.valid_paths)} test WSI's.")
        ...
        if past_coords:
            dataset, past_coords, past_wsi, save_image = SurfSampler.tester(self, image, mask_image, img_reg, mask_reg,
                                                                            numpy_batch_patch, numpy_batch_mask,
                                                                            save_image, past_coords, past_wsi)
        else:
            dataset, past_coords, past_wsi, save_image = SurfSampler.trainer(self, image, mask_image, img_reg, mask_reg,
                                                                             numpy_batch_patch, numpy_batch_mask,
                                                                             save_image)
