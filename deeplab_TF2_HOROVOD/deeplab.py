...
def ASPP(tensor):
    ...
    y_6 = tf.reshape(y_6,[-1,y_pool.shape[1],y_pool.shape[2],y_pool.shape[3]])
    ...
    y_12 = tf.reshape(y_12, [-1, y_pool.shape[1], y_pool.shape[2], y_pool.shape[3]])

    y_18 = Conv2D(filters=256, kernel_size=3, dilation_rate=18, padding='same',
                  kernel_initializer='he_normal', name='ASPP_conv2d_d18', use_bias=False)(tensor)
    y_18 = BatchNormalization(name=f'bn_5')(y_18)
    y_18 = Activation('relu', name=f'relu_5')(y_18)
    y_18 = tf.reshape(y_18, [-1, y_pool.shape[1], y_pool.shape[2], y_pool.shape[3]])
